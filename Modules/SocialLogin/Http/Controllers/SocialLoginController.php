...
use Modules\SocialLogin\Http\Requests\SocialLoginRequest;
use Carbon\Carbon;
use Exception;
...
class SocialLoginController extends DboardController
{

    public function redirectToProvider(string $provider, Request $request, DboardHttpResponse $response)
    {
    ...
    public function handleProviderCallback(string $provider, DboardHttpResponse $response)
    {
        $guard = $this->guard();
        ...
        if (! $account) {
            ...
            try {
                $url = $oAuth->getAvatar();
                if ($url) {
//                    $result = uploadFromUrl($url, 0, $model->upload_folder ?: 'accounts', 'image/png');
//                    if (! $result['error']) {
...
//                    }
        ...
        return $response
            ...
            ->setMessage(trans('kamruldashboard::auth.login.success'));
    }

    public function getSettings()
    {
        page_title()->setTitle(trans('sociallogin::social-login.settings.title'));

        Assets::addScriptsDirectly('vendor/Modules/SocialLogin/js/social-login.js');

        return view('sociallogin::settings');
    }

    public function postSettings(SocialLoginRequest $request, DboardHttpResponse $response, SettingStore $setting)
    {
