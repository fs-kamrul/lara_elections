...
@section('content')
    ...
    <div class="max-width-1200">
        <div class="flexbox-annotated-section">
            ...
            <div class="flexbox-annotated-section-annotation">
                <div class="annotated-section-title pd-all-20">
                    <h2>{{ trans('sociallogin::social-login.settings.title') }}</h2>
                </div>
                <div class="annotated-section-description pd-all-20 p-none-t">
                    <p class="color-note">{{ trans('sociallogin::social-login.settings.description') }}</p>
                </div>
            </div>
                ...
                <div class="wrapper-content pd-all-20">
                    <div class="form-group mb-0">
                        ...
                        <label>
                            ...
                            {{ trans('sociallogin::social-login.settings.enable') }}
                        </label>
                    </div>
        ...
        <div class="wrapper-list-social-login-options" @if (!SocialService::setting('enable')) style="display:none;" @endif>
            @foreach (SocialService::getProviders() as $provider => $item)
                <div class="flexbox-annotated-section">
                    ...
                    <div class="flexbox-annotated-section-annotation">
                        <div class="annotated-section-title pd-all-20">
                            <h2>{{ trans('sociallogin::social-login.settings.' . $provider . '.title') }}</h2>
                        </div>
                        <div class="annotated-section-description pd-all-20 p-none-t">
                            <p class="color-note">{{ trans('sociallogin::social-login.settings.' . $provider . '.description') }}</p>
                        </div>
                    </div>
                                ...
                                @foreach ($item['data'] as $input)
                                    @if (in_array(app()->environment(), SocialService::getEnvDisableData()) && in_array($input, Arr::get($item, 'disable', [])))
                                        <div class="form-group mb-3">
                                            <label class="text-title-field">{{ trans('sociallogin::social-login.settings.' . $provider . '.' . $input) }}</label>
                                            <input type="text" disabled readonly class="next-input" value="{{ SocialService::getDataDisable($provider . '_' . $input) }}">
                                        </div>
                                    @else
                                        <div class="form-group mb-3">
                                            <label class="text-title-field"
                                                for="social_login_{{ $provider }}_{{ $input }}">{{ trans('sociallogin::social-login.settings.' . $provider . '.' . $input) }}</label>
                                    ...
                                    @endif
                                @endforeach
                                {!! Form::helper(trans('sociallogin::social-login.settings.' . $provider . '.helper', ['callback' => '<code>' . route('auth.social.callback', $provider) . '</code>'])) !!}
                            </div>
        ...
        <div class="flexbox-annotated-section" style="border: none">
            ...
            <div class="flexbox-annotated-section-content">
                <button class="btn btn-info" type="submit">{{ trans('kamruldashboard::lang.save_settings') }}</button>
            </div>
        </div>
    </div>
    {!! Form::close() !!}
