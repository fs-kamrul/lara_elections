...
<div class="card">
    ...
    <div class="card-body">
        <div class="basic-form">
            ...
            $button = 'update';
            @endphp
            <form name="formmenus" method="POST" action="{{ route('menus.store_import', $record->id) }}" novalidate=""  enctype="multipart/form-data">
                {{ method_field("PATCH") }}
                @else
                @php
                $button = 'save';
                @endphp
                <form method="POST" action="{{ route('menus.store_import') }}"  enctype="multipart/form-data">
                    @endif
                    @csrf

                    <div class="form-row">

                        @isset($record){!! input_design_html('photo',$record,6,'file', __('menus::lang.import')) !!} @else{!! input_design_html('photo',null,6,'file', __('menus::lang.import')) !!} @endisset
        ...
        <button type="submit" class="btn btn-primary">{{ __('menus::lang.'.$button) }}</button>
        </form>
    </div>
</div>
</div>
