...
<div class="card">
    ...
    <div class="card-body">
        <div class="basic-form">
            ...
            $button = 'update';
            @endphp
            <form name="formsimpleslider" method="POST" action="{{ route('simpleslider.store_import', $record->id) }}" novalidate=""  enctype="multipart/form-data">
                {{ method_field("PATCH") }}
                @else
                @php
                $button = 'save';
                @endphp
                <form method="POST" action="{{ route('simpleslider.store_import') }}"  enctype="multipart/form-data">
                    @endif
                    @csrf

                    <div class="form-row">

                        @isset($record){!! input_design_html('photo',$record,6,'file', __('simpleslider::lang.import')) !!} @else{!! input_design_html('photo',null,6,'file', __('simpleslider::lang.import')) !!} @endisset
        ...
        <button type="submit" class="btn btn-primary">{{ __('simpleslider::lang.'.$button) }}</button>
        </form>
    </div>
</div>
</div>
