...
namespace Modules\VenueFacility\Tables;
...
class VenueFacilityTable extends TableAbstract
{
    /**
     * @var bool
     */
    ...
    /**
     * VenueFacilityTable constructor.
     * @param DataTables $table
     * @param UrlGenerator $urlGenerator
     * @param VenueFacilityInterface $venuefacilityRepository
     */
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, VenueFacilityInterface $venuefacilityRepository)
    {
        parent::__construct($table, $urlGenerator);

        $this->repository = $venuefacilityRepository;

        if (!Auth::user()->can(['venuefacility_edit', 'venuefacility_destroy'])) {
        ...
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (! Auth::user()->can('venuefacility_edit')) {
                    ...
                    $name = Html::link(route('venuefacility.edit', $item->id), DboardHelper::clean($item->name));
                }

                return $name;
            })
            ->editColumn('photo', function ($item) {
                $photo_path = 'venuefacility';
            ...
            ->addColumn('operations', function ($item) {
                return $this->getOperations('venuefacility.edit', 'venuefacility.destroy', $item);
            });
    ...
    {
        return $this->addCreateButton(route('venuefacility.create'), 'venuefacility_create');
    }

    public function bulkActions(): array
    {
        return $this->addDeleteAction(route('venuefacility.deletes'), 'venuefacility_destroy', parent::bulkActions());
    }
