...
@if(isset($record))
    @php
        $button = 'update';
        $language = getLanguageUrlPost($record->id , 'optionyear');
    @endphp
    <form name="formPage" method="POST" action="{{ $language['url'] }}" novalidate=""  enctype="multipart/form-data">
        <input type="hidden" name="model" value="{{ \Modules\Option\Http\Models\OptionYear::class }}">
        <input type="hidden" name="language" value="{{ $language['code'] }}">
@else
    @php
        $button = 'save';
    @endphp
    <form method="POST" action="{{ route('optionyear.createoptionyear.store') }}"  enctype="multipart/form-data">
@endif
@csrf
        @php do_action(BASE_ACTION_TOP_FORM_CONTENT_NOTIFICATION, request(), \Modules\Option\Http\Models\OptionYear::class) @endphp

    <div class="row">
        <div class=" col-md-9">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">@lang('option::lang.optionyear_create')</h4>
                </div>
                <div class="card-body">
                    <div class="basic-form">
                        <div class="form-row">
                            ...
                            @php
                                if(isset($record)){
                                    $slug = get_slug_table_data(null, \Modules\Option\Http\Models\OptionYear::class,$record->id, \Modules\Option\Http\Models\OptionYear::class);
                                }
                            @endphp
...
{{--                            </div>--}}
        ...
        <div class=" col-md-3">
            ...
            @isset($record)
                @php do_action(BASE_ACTION_META_BOXES, 'top', \Modules\Option\Http\Models\OptionYear::where('id',$record->id)->first()) @endphp
            @endisset
        </div>
    </div>
    </form>
