...
class OptionYearTable extends TableAbstract
{
    /**
     * @var bool
     */
    ...
    /**
     * OptionYearTable constructor.
     * @param DataTables $table
     * @param UrlGenerator $urlGenerator
     * @param OptionYearInterface $optionyearRepository
     */
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, OptionYearInterface $optionyearRepository)
    {
        parent::__construct($table, $urlGenerator);

        $this->repository = $optionyearRepository;

        if (!Auth::user()->can(['optionyear_edit', 'optionyear_destroy'])) {
        ...
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (! Auth::user()->can('optionyear_edit')) {
                    ...
                    $name = Html::link(route('optionyear.edit', $item->id), DboardHelper::clean($item->name));
                }

                return $name;
            })
            ->editColumn('photo', function ($item) {
                $photo_path = 'optionyear';
            ...
            ->addColumn('operations', function ($item) {
                return $this->getOperations('optionyear.edit', 'optionyear.destroy', $item);
            });
    ...
    {
        return $this->addCreateButton(route('optionyear.create'), 'optionyear_create');
    }

    public function bulkActions(): array
    {
        return $this->addDeleteAction(route('optionyear.deletes'), 'optionyear_destroy', parent::bulkActions());
    }
