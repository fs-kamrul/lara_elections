...
class AdminWorkshopRepository extends RepositoriesAbstract implements AdminWorkshopInterface
{
    public function getWorkshop($filters = [], $params = [])
    {
        $filters = array_merge([
            'keyword' => null,
            'min_floor' => null,
...
//        }
        ...
        $this->model = apply_filters('workshop_filter_query', $this->model, $filters, $params);

        return $this->advancedGet($params);
    }

    public function getRelatedWorkshop(int $projectId, int $limit = 4, array $with = [])
    {
...
//            $categoryIds = $currentWorkshop->categories->pluck('id')->toArray();
//
//            $this->model
//                ->whereHas('categories', function ($query) use ($categoryIds) {
//                    $query->whereIn('workshop_categories.category_id', $categoryIds);
//                });
//        }
