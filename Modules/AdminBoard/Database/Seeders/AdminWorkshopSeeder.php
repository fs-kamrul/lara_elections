...
class AdminWorkshopSeeder extends Seeder
{
    /**
...
//        ];
        for ($i = 1; $i <= 24; $i++) {
            ...
            $data_slug[] = [
                'key'               => $slug,
                'reference_id'      => $i,
                'reference_type'    => AdminWorkshop::class,
                'prefix'            => SlugHelper::getPrefix(AdminWorkshop::class),
            ];
        }
        Slug::where('reference_type',AdminWorkshop::class)->delete();
//        SlugInterface::
//        print_r($data_slug);
        AdminWorkshop::upsert($data, ['name']);
