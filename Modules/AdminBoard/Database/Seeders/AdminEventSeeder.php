...
//        ];
        for ($i = 1; $i <= 36; $i++) {
            ...
            $data_slug[] = [
                'key'               => $slug,
                'reference_id'      => $i,
                'reference_type'    => AdminEvent::class,
                'prefix'            => SlugHelper::getPrefix(AdminEvent::class),
            ];
        }
        Slug::where('reference_type',AdminEvent::class)->delete();
//        SlugInterface::
//        print_r($data_slug);
        AdminEvent::upsert($data, ['name']);
