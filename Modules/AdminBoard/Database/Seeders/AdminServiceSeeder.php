...
//        ];
        for ($i = 1; $i <= 6; $i++) {
            ...
            $data_slug[] = [
                'key'               => $slug,
                'reference_id'      => $i,
                'reference_type'    => AdminService::class,
                'prefix'            => SlugHelper::getPrefix(AdminService::class),
            ];
        }
        Slug::where('reference_type',AdminService::class)->delete();
//        SlugInterface::
//        print_r($data_slug);
        AdminService::upsert($data, ['name']);
