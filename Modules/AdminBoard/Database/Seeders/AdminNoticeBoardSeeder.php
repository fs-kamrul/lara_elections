...
class AdminNoticeBoardSeeder extends Seeder
{
    /**
...
//        ];
        for ($i = 1; $i <= 24; $i++) {
            $name = $faker->sentence(12); // Generates a title with 3 words
            $slug = Str::slug($name);
            ...
            $data_slug[] = [
                'key'               => $slug,
                'reference_id'      => $i,
                'reference_type'    => AdminNoticeBoard::class,
                'prefix'            => SlugHelper::getPrefix(AdminNoticeBoard::class),
            ];
        }
        Slug::where('reference_type',AdminNoticeBoard::class)->delete();
//        SlugInterface::
//        print_r($data_slug);
        AdminNoticeBoard::upsert($data, ['name']);
