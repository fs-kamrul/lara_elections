...
@section('content')
    ...
    @if(isset($record))
        @php
            $button = 'update';
            $language = getLanguageUrlPost($record->id , 'adminservice');
        @endphp
        <form name="formPage" method="POST" action="{{ $language['url'] }}" novalidate=""  enctype="multipart/form-data">
            <input type="hidden" name="model" value="{{ \Modules\AdminBoard\Http\Models\AdminService::class }}">
            <input type="hidden" name="language" value="{{ $language['code'] }}">
            @else
                @php
                    $button = 'save';
                @endphp
                <form method="POST" action="{{ route('adminservice.createadminservice.store') }}"  enctype="multipart/form-data">
                    @endif
                    @csrf
                    @php do_action(BASE_ACTION_TOP_FORM_CONTENT_NOTIFICATION, request(), \Modules\AdminBoard\Http\Models\AdminService::class) @endphp

                    <div class="row">
                        <div class=" col-md-9">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">@lang('adminboard::lang.adminservicecreate')</h4>
                                </div>
                                <div class="card-body">
                                    <div class="basic-form">
                                        <div class="form-row">
                                            ...
                                            @php
                                                if(isset($record)){
                                                    $slug = get_slug_table_data(null, \Modules\AdminBoard\Http\Models\AdminService::class,$record->id, \Modules\AdminBoard\Http\Models\AdminService::class);
                                                }
                                            @endphp
                            ...
                            @isset($record)
                                @php do_action(BASE_ACTION_META_BOXES, 'advanced', \Modules\AdminBoard\Http\Models\AdminService::where('id',$record->id)->first()) @endphp
                            @else
                                @php do_action(BASE_ACTION_META_BOXES, 'advanced', new \Modules\AdminBoard\Http\Models\AdminService ) @endphp
                            @endisset
                        </div>
                        <div class=" col-md-3">
                            @isset($record)
                                        ...
                                        <div class="form-row">
                                            @isset($record)
                                                @if($language['code_edit'])
                                                    {!! status_admin_enum($record->status,12, '', \Modules\AdminBoard\Enums\AdminServiceStatusEnum::class) !!}
                                                @endif @else {!! status_admin_enum(1,12, '' , \Modules\AdminBoard\Enums\AdminServiceStatusEnum::class) !!}
                                            @endisset
                                        </div>
                                    </div>
                                </div>
                            ...
                            @isset($record)
                                @php do_action(BASE_ACTION_META_BOXES, 'top', \Modules\AdminBoard\Http\Models\AdminService::where('id',$record->id)->first()) @endphp
                            @else
                                @php do_action(BASE_ACTION_META_BOXES, 'top', new \Modules\AdminBoard\Http\Models\AdminService ) @endphp
                            @endisset
                        </div>
                    </div>
                </form>
