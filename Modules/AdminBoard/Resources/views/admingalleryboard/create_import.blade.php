...
<div class="card">
    <div class="card-header">
        <h4 class="card-title">@lang('adminboard::lang.admingalleryboard_create')</h4>
    </div>
    <div class="card-body">
        <div class="basic-form">
            ...
            $button = 'update';
            @endphp
            <form name="formadmingalleryboard" method="POST" action="{{ url('adminboard/admingalleryboard/store_import', $record->id) }}" novalidate=""  enctype="multipart/form-data">
                {{ method_field("PATCH") }}
                @else
                @php
                $button = 'save';
                @endphp
                <form method="POST" action="{{ url('adminboard/admingalleryboard/store_import') }}"  enctype="multipart/form-data">
                    @endif
                    @csrf
