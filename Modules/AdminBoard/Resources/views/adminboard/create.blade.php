...
@if(isset($record))
    @php
        $button = 'update';
        $language = getLanguageUrlPost($record->id , 'adminboard');
    @endphp
    <form name="formPage" method="POST" action="{{ $language['url'] }}" novalidate=""  enctype="multipart/form-data">
        <input type="hidden" name="model" value="{{ \Modules\AdminBoard\Http\Models\AdminBoard::class }}">
        <input type="hidden" name="language" value="{{ $language['code'] }}">
@else
    @php
        $button = 'save';
    @endphp
    <form method="POST" action="{{ route('adminboard.createadminboard.store') }}"  enctype="multipart/form-data">
@endif
@csrf
        @php do_action(BASE_ACTION_TOP_FORM_CONTENT_NOTIFICATION, request(), \Modules\AdminBoard\Http\Models\AdminBoard::class) @endphp
    <div class="row">
        <div class=" col-md-9">
            <div class="card">
                <div class="card-header">
                    <h4 class="card-title">@lang('adminboard::lang.adminboard_create')</h4>
                </div>
                <div class="card-body">
                    <div class="basic-form">
                        <div class="form-row">
                            ...
                            @php
                                if(isset($record)){
                                    $slug = get_slug_table_data(null, \Modules\AdminBoard\Http\Models\AdminBoard::class,$record->id, \Modules\AdminBoard\Http\Models\AdminBoard::class);
                                }
                            @endphp
                            ...
                            @isset($record) @if($language['code_edit']){!! status_design_html($record->status,6, __('adminboard::lang.status')) !!} @endif @else{!! status_design_html(1,6, __('adminboard::lang.status')) !!} @endisset

                            <div class="col-xl-12 col-xxl-12">
                                <label><strong>@lang('adminboard::lang.description')</strong></label>
        ...
        <div class=" col-md-3">
            ...
            @isset($record)
                @php do_action(BASE_ACTION_META_BOXES, 'top', \Modules\AdminBoard\Http\Models\AdminBoard::where('id',$record->id)->first()) @endphp
            @endisset
        </div>
    </div>
    </form>
