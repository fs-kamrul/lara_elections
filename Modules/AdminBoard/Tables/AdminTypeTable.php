...
class AdminTypeTable extends TableAbstract
{
    /**
     * @var bool
     */
    ...
    /**
     * AdminTypeTable constructor.
     * @param DataTables $table
     * @param UrlGenerator $urlGenerator
     * @param AdminTypeInterface $admintypeRepository
     */
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, AdminTypeInterface $admintypeRepository)
    {
        parent::__construct($table, $urlGenerator);

        $this->repository = $admintypeRepository;

        if (!Auth::user()->can(['admintype_edit', 'admintype_destroy'])) {
        ...
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (! Auth::user()->can('admintype_edit')) {
                    ...
                    $name = Html::link(route('admintype.edit', $item->id), DboardHelper::clean($item->name));
                }

                return $name;
            })
            ->editColumn('photo', function ($item) {
                $photo_path = 'adminboard/admintype';
            ...
            ->addColumn('operations', function ($item) {
                return $this->getOperations('admintype.edit', 'admintype.destroy', $item);
            });
    ...
    {
        return $this->addCreateButton(route('admintype.create'), 'admintype_create');
    }

    public function bulkActions(): array
    {
        return $this->addDeleteAction(route('admintype.deletes'), 'admintype_destroy', parent::bulkActions());
    }
    ...
    public function getBulkChanges(): array
    {
        return [
            ...
            'status' => [
                'title' => trans('table::lang.status'),
                'type' => 'customSelect',
                'choices' => AdminTypeStatusEnum::labels(),
                'validate' => 'required',
            ],
            'created_at' => [
                'title' => trans('table::lang.created_at'),
