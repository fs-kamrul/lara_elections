...
class AdminPartnerTable extends TableAbstract
{
     ...
     */
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, AdminPartnerInterface $adminpartnerRepository, AdminCategoryInterface $adminCategoryRepository)
    {
    ...
    {
        return $this->adminCategoryRepository->getCategoriesByBoard('partner');
    }
    public function applyFilterCondition($query, string $key, string $operator, ?string $value)
    {
        switch ($key) {
            ...
            case 'category':
                ...
                if (!DboardHelper::isJoined($query, 'admin_partner_categories')) {
                    $query = $query
                        ->join('admin_partner_categories', 'admin_partner_categories.team_id', '=', 'admin_partners.id')
                        ->join('admin_categories', 'admin_partner_categories.category_id', '=', 'admin_categories.id')
                        ->select($query->getModel()->getTable() . '.*');
                }

                return $query->where('admin_partner_categories.category_id', $value);
        }
