<div class="flexbox-annotated-section">
    <div class="flexbox-annotated-section-annotation">
        <div class="annotated-section-title pd-all-20">
            <h2>{{ trans('newsletter::newsletter.settings.title') }}</h2>
        </div>
        <div class="annotated-section-description pd-all-20 p-none-t">
            <p class="color-note">{{ trans('newsletter::newsletter.settings.description') }}</p>
        </div>
    </div>
        ...
        <div class="wrapper-content pd-all-20">
            <div class="form-group mb-3">
                <label class="text-title-field"
                       for="enable_cache">{{ trans('newsletter::newsletter.settings.enable_newsletter_contacts_list_api') }}
                </label>
                <label class="me-2">
            ...
            <div id="newsletter-settings" @class(['mb-4 border rounded-top rounded-bottom p-3 bg-light', 'd-none' => ! setting('enable_newsletter_contacts_list_api')])>
                <div class="form-group mb-3">
                    <label class="text-title-field"
                           for="newsletter_mailchimp_api_key">{{ trans('newsletter::newsletter.settings.mailchimp_api_key') }}</label>
                    <input data-counter="120" type="text" class="next-input" name="newsletter_mailchimp_api_key"
                           ...
                           placeholder="{{ trans('newsletter::newsletter.settings.mailchimp_api_key') }}">
                </div>
                <div class="form-group mb-3">
                    @if (empty($mailchimpContactList))
                        <label class="text-title-field"
                               for="newsletter_mailchimp_list_id">{{ trans('newsletter::newsletter.settings.mailchimp_list_id') }}</label>
                        <input data-counter="120" type="text" class="next-input" name="newsletter_mailchimp_list_id"
                               ...
                               placeholder="{{ trans('newsletter::newsletter.settings.mailchimp_list_id') }}">
                    @else
                        <label class="text-title-field"
                               for="newsletter_mailchimp_list_id">{{ trans('newsletter::newsletter.settings.mailchimp_list') }}</label>
                ...
                <div class="form-group mb-3">
                    <label class="text-title-field"
                           for="newsletter_sendgrid_api_key">{{ trans('newsletter::newsletter.settings.sendgrid_api_key') }}</label>
                    <input data-counter="120" type="text" class="next-input" name="newsletter_sendgrid_api_key"
                           ...
                           placeholder="{{ trans('newsletter::newsletter.settings.sendgrid_api_key') }}">
                </div>
                <div class="form-group mb-3">
                    @if (empty($sendGridContactList))
                        <label class="text-title-field"
                               for="newsletter_sendgrid_list_id">{{ trans('newsletter::newsletter.settings.sendgrid_list_id') }}</label>
                        <input data-counter="120" type="text" class="next-input" name="newsletter_sendgrid_list_id"
                               ...
                               placeholder="{{ trans('newsletter::newsletter.settings.sendgrid_list_id') }}">
                    @else
                        <label class="text-title-field"
                               for="newsletter_sendgrid_list_id">{{ trans('newsletter::newsletter.settings.sendgrid_list') }}</label>
