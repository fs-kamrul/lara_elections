...
<div class="card">
    ...
    <div class="card-body">
        <div class="basic-form">
            ...
            $button = 'update';
            @endphp
            <form name="formcaptcha" method="POST" action="{{ route('captcha.store_import', $record->id) }}" novalidate=""  enctype="multipart/form-data">
                {{ method_field("PATCH") }}
                @else
                @php
                $button = 'save';
                @endphp
                <form method="POST" action="{{ route('captcha.store_import') }}"  enctype="multipart/form-data">
                    @endif
                    @csrf

                    <div class="form-row">

                        @isset($record){!! input_design_html('photo',$record,6,'file', __('captcha::lang.import')) !!} @else{!! input_design_html('photo',null,6,'file', __('captcha::lang.import')) !!} @endisset
        ...
        <button type="submit" class="btn btn-primary">{{ __('captcha::lang.'.$button) }}</button>
        </form>
    </div>
</div>
</div>
