...
<div class="card">
    ...
    <div class="card-body">
        <div class="basic-form">
            ...
            $button = 'update';
            @endphp
            <form name="formthemeicon" method="POST" action="{{ route('themeicon.store_import', $record->id) }}" novalidate=""  enctype="multipart/form-data">
                {{ method_field("PATCH") }}
                @else
                @php
                $button = 'save';
                @endphp
                <form method="POST" action="{{ route('themeicon.store_import') }}"  enctype="multipart/form-data">
                    @endif
                    @csrf

                    <div class="form-row">

                        @isset($record){!! input_design_html('photo',$record,6,'file', __('themeicon::lang.import')) !!} @else{!! input_design_html('photo',null,6,'file', __('themeicon::lang.import')) !!} @endisset
