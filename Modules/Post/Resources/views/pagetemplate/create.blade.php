...
@if(isset($record))
    @php
        $button = 'update';
        $language = getLanguageUrlPost($record->id , 'pagetemplate');
//    dd($language['code']);
    @endphp
    <form name="formPage" method="POST" action="{{ $language['url'] }}" novalidate=""  enctype="multipart/form-data">
        <input type="hidden" name="model" value="{{ \Modules\Post\Http\Models\PageTemplate::class }}">
        <input type="hidden" name="language" value="{{ $language['code'] }}">
@else
    @php
        $button = 'save';
    @endphp
    <form method="POST" action="{{ route('pagetemplate.createpagetemplate.store') }}"  enctype="multipart/form-data">
@endif
@csrf
