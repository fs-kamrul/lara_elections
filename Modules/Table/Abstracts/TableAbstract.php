...
abstract class TableAbstract extends DataTable
{
            ...
            ->parameters([
                ...
                'lengthMenu'   => [
                    ...
                    array_values(array_unique(array_merge([10, 30, 50, 100, 500],
                        [$this->pageLength, trans('table::lang.all')]))),
                ],
                ...
                'language'     => [
                    ...
                    'emptyTable'        => trans('table::lang.no_data'),
                    'info'              => view('table::table-info')->render(),
                    'infoEmpty'         => trans('table::lang.no_record'),
                    'lengthMenu'        => Html::tag('span', '_MENU_', ['class' => 'dt-length-style'])->toHtml(),
                    'search'            => '',
                    'searchPlaceholder' => trans('table::lang.search'),
                    'zeroRecords'       => trans('table::lang.no_record'),
                    'processing'        => Html::image(url('vendor/Modules/KamrulDashboard/images/loading-spinner-blue.gif')),
                    'paginate'          => [
                        'next'     => trans('pagination.next'),
                        'previous' => trans('pagination.previous'),
                    ],
                    'infoFiltered'      => trans('table::lang.filtered'),
                ],
    ...
    public function getOperationsHeading()
    {
        return [
            'operations' => [
                'title'      => trans('table::lang.operations'),
                'width'      => '134px',
                'class'      => 'text-center',
                'orderable'  => false,
                'searchable' => false,
    ...
    public function getActionsButton(): array
    {
        ...
        return [
            [
                'extend'  => 'collection',
                'text'    => '<span>' . trans('kamruldashboard::lang.actions') . ' <span class="caret"></span></span>',
    ...
    public function render($view, $data = [], $mergeData = [])
    {
        Assets::addScripts(['datatables', 'moment', 'datepicker'])
            ...
            ->addStylesDirectly('vendor/Modules/Table/css/table.css')
            ->addScriptsDirectly([
                'vendor/Modules/KamrulDashboard/vendor2/Bootstrap-3-Typeahead/bootstrap3-typeahead.min.js',
                'vendor/Modules/Table/js/table.js',
                'vendor/Modules/Table/js/filter.js',
            ]);
    ...
    public function getValueInput(?string $title, ?string $value, ?string $type, $data = null): array
    {
        ...
        $attributes = [
            ...
            'placeholder'  => trans('table::lang.value'),
            'autocomplete' => 'off',
        ];
        ...
        switch ($type) {
            ...
            case 'customSelect':
                ...
                $attributes['placeholder'] = trans('table::lang.select_option');
            ...
            case 'date':
                ...
                $attributes['data-date-format'] = config('kamruldashboard.date_format.js.date');
    ...
    public function prepareBulkChangeValue(string $key, ?string $value): string
    {
        ...
        switch ($key) {
            case 'created_at':
            case 'updated_at':
                $value = DboardHelper::formatDateTime($value);
                break;
        }
    ...
    protected function addCreateButton(string $url, $permission = null, array $buttons = []): array
    {
        if (!$permission || Auth::user()->can($permission)) {
            ...
            $buttons['create'] = [
                'link' => $url,
                'text' => view('table::partials.create')->render(),
            ];
        }
