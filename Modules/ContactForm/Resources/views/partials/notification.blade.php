<li class="dropdown dropdown-extended dropdown-inbox">
    ...
    <ul class="dropdown-menu dropdown-menu-right">
        <li class="external">
            <h3>{!! clean(trans('contactform::contact.new_msg_notice', ['count' => $contacts->total()])) !!}</h3>
            <a href="{{ route('contactform.index') }}">{{ trans('contactform::contact.view_all') }}</a>
        </li>
        <li>
            <ul class="dropdown-menu-list scroller" style="height: {{ $contacts->total() * 70 }}px;" data-handle-color="#637283">
                @foreach($contacts as $contact)
                    <li>
                        <a href="{{ route('contactform.edit', $contact->id) }}">
                            <span class="photo">
                ...
                @if ($contacts->total() > 10)
                    <li class="text-center"><a href="{{ route('contactform.index') }}">{{ trans('contactform::contact.view_all') }}</a></li>
                @endif
            </ul>
        </li>
    </ul>
