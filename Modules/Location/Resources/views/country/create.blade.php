...
@if(isset($record))
    @php
        $button = 'update';
        $language = getLanguageUrlPost($record->id , 'country');
    @endphp
    <form name="formPage" method="POST" action="{{ $language['url'] }}" novalidate=""  enctype="multipart/form-data">
        <input type="hidden" name="model" value="{{ \Modules\Location\Http\Models\Country::class }}">
        <input type="hidden" name="language" value="{{ $language['code'] }}">
@else
    @php
        $button = 'save';
    @endphp
    <form method="POST" action="{{ route('country.createcountry.store') }}"  enctype="multipart/form-data">
@endif
@csrf
        @php do_action(BASE_ACTION_TOP_FORM_CONTENT_NOTIFICATION, request(), \Modules\Location\Http\Models\Country::class) @endphp
    ...
    <div class="row">
                ...
                <div class="card-body">
                    <div class="basic-form">
                        <div class="form-row">
                            ...
                            {!! Form::textField('code', isset($record) ? $record->code : null, 'location', '12', ['placeholder'=> __('location::lang.country_iso_code')],
                            trans(
                                     'location::lang.code_helper'
                                 )
                            ) !!}
                            @endif
                            {!! Form::textField('nationality', isset($record) ? $record->nationality : null, 'location', '12', ['placeholder'=> __('location::country.nationality')]) !!}

                            @isset($record) @if($language['code_edit']){!! status_design_html($record->status,6, __('location::lang.status')) !!} @endif @else{!! status_design_html(1,6, __('location::lang.status')) !!} @endisset
                            @if(isset($record) ? $language['code_edit'] : 1)
        ...
        <div class=" col-md-3">
            ...
            @endif
            @isset($record)
                @php do_action(BASE_ACTION_META_BOXES, 'top', \Modules\Location\Http\Models\Country::where('id',$record->id)->first()) @endphp
            @endisset
        </div>
    </div>
    </form>
