...
@if(isset($record))
    @php
        $button = 'update';
        $language = getLanguageUrlPost($record->id , 'state');
    @endphp
    <form name="formPage" method="POST" action="{{ $language['url'] }}" novalidate=""  enctype="multipart/form-data">
        <input type="hidden" name="model" value="{{ \Modules\Location\Http\Models\State::class }}">
        <input type="hidden" name="language" value="{{ $language['code'] }}">
@else
    @php
        $button = 'save';
    @endphp
    <form method="POST" action="{{ route('state.createstate.store') }}"  enctype="multipart/form-data">
@endif
@csrf
        @php do_action(BASE_ACTION_TOP_FORM_CONTENT_NOTIFICATION, request(), \Modules\Location\Http\Models\State::class) @endphp
    ...
    <div class="row">
                ...
                <div class="card-body">
                    <div class="basic-form">
                        <div class="form-row">
                            ...
                            {!! Form::textField('abbreviation', isset($record) ? $record->abbreviation : null, 'location', '12', ['placeholder'=> __('location::lang.abbreviation')]) !!}
                            ...
                            @if(isset($record) ? $language['code_edit'] : 1)
                                <div class="form-group">
                                    <label for="widget_menu">{{ __('Country') }}</label>
        ...
        <div class=" col-md-3">
            ...
            @endif
            @isset($record)
                @php do_action(BASE_ACTION_META_BOXES, 'top', \Modules\Location\Http\Models\State::where('id',$record->id)->first()) @endphp
            @endisset
        </div>
    </div>
    </form>
