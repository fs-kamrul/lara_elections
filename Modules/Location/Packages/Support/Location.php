...
namespace Modules\Location\Packages\Support;
...
class Location
{
    ...
    public function __construct(StateInterface $stateRepository, CityInterface $cityRepository, CountryInterface $countryRepository)
    {
    ...
    {
        $states = $this->countryRepository->advancedGet([
            'condition' => [
    ...
    public function getNationality(): array
    {
        ...
        return $states->pluck('nationality', 'id')->all();
    }
    ...
    public function getNationalityNameById($stateId): ?string
    {
        $state = $this->countryRepository->getFirstBy([
            'id' => $stateId,
            'status' => DboardStatus::PUBLISHED,
        ]);

        return $state ? $state->nationality : null;
    }
