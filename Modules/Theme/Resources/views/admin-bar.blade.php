<nav id="admin_bar">
    <div class="admin-bar-container">
        <div class="admin-bar-logo">
            <a href="{{ route('dashboard.index') }}" title="{{ trans('theme::theme.go_to_dashboard') }}">
{{--                <img src="{{ setting('admin_logo') ? RvMedia::getImageUrl(setting('admin_logo')) : url(config('core.base.general.logo')) }}" alt="logo"/>--}}
                <img class="brand-title" src="{{ url('uploads/settings/'.getSetting('site_logo', 'site_setting')) }}" alt="">
            </a>
        </div>
        ...
        <ul class="admin-navbar-nav admin-navbar-nav-right">
            <li class="admin-bar-dropdown">
                ...
                <ul class="admin-bar-dropdown-menu">
                    <li><a href="{{ route('users.profile.view', Auth::id()) }}">{{ trans('theme::layouts.profile') }}</a></li>
                    <li><a href="{{ route('access.logout') }}">{{ trans('theme::layouts.logout') }}</a></li>
                </ul>
            </li>
        </ul>
    </div>
