...
use Modules\KamrulDashboard\Packages\Supports\PluginService;
...
//        $this->widgetRepository = $widgetRepository;
    ...
    public function activate(string $theme): array
    {
        ...
        if (setting('theme') && $theme == Theme::getThemeName()) {
            return [
                'error'   => true,
                'message' => trans('theme::theme.theme_activated_already', ['name' => $theme]),
            ];
        }
...
//        Artisan::call('route:cache');
        return [
            'error'   => false,
            'message' => trans('theme::theme.active_success', ['name' => $theme]),
        ];
    }
    ...
    protected function validate(string $theme): array
    {
        ...
        if (!$this->files->isDirectory($location)) {
            return [
                'error'   => true,
                'message' => trans('theme::theme.theme_is_not_existed'),
            ];
        }
        ...
        if (!$this->files->exists($location . '/theme.json')) {
            return [
                'error'   => true,
                'message' => trans('theme::theme.missing_json_file'),
            ];
        }

        return [
            'error'   => false,
            'message' => trans('theme::theme.theme_invalid'),
        ];
    }
    ...
    {
        return rtrim(theme_path(), '/') . '/' . rtrim(ltrim($theme, '/'), '/') . '/' . $path;
    }
    ...
    public function publishAssets(string $theme = null): array
    {
        ...
        foreach ($themes as $theme) {
            ...
            $themePath = public_path(config('theme.themeDir') . '/' . $theme);
//            $themePath = public_path($theme);
            if (!$this->files->isDirectory($themePath)) {
                $this->files->makeDirectory($themePath, 0755, true);
            } elseif (!$this->files->isWritable($themePath)) {
                return [
                    'error'   => true,
                    'message' => trans('theme::theme.folder_is_not_writeable', ['name' => $themePath]),
                ];
            }
...
//            }
            ...
            $this->files->copyDirectory($resourcePath, $themePath);
            $this->files->copy($this->getPath($theme, 'screenshot.png'), $themePath . '/screenshot.png');
        }

        return [
            'error'   => false,
            'message' => trans('theme::theme.published_assets_success', ['themes' => implode(', ', $themes)]),
        ];
    }
    ...
    public function remove(string $theme): array
    {
        ...
        if (Theme::getThemeName() == $theme) {
            return [
                'error'   => true,
                'message' => trans('theme::theme.cannot_remove_theme', ['name' => $theme]),
            ];
        }
...
//        $this->widgetRepository->deleteBy(['theme' => $theme]);
        ...
        return [
            'error'   => false,
            'message' => trans('theme::theme.theme_deleted', ['name' => $theme]),
        ];
    }
    ...
    public function removeAssets(string $theme): array
    {
        ...
        $this->files->deleteDirectory(public_path($theme));

        return [
            'error'   => false,
            'message' => trans('theme::theme.removed_assets', ['name' => $theme]),
        ];
    }
}
