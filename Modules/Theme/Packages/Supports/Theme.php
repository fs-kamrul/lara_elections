...
//            'Activated-License' => !empty(setting('licensed_to')) ? 'Yes' : 'No',
    ...
    public function registerRoutes(Closure $closure): void
    {
        Route::group(['middleware' => ['web']], function () use ($closure) {
            Route::group(apply_filters(FILTER_GROUP_PUBLIC_ROUTE, []), function () use ($closure) {
                $closure();
            });
        });
    }
