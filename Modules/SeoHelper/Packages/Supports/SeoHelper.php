...
namespace Modules\SeoHelper\Packages\Supports;
...
class SeoHelper implements SeoHelperContract
{
     ...
     * @param DboardModel $object
     * @return bool
     */
    public function saveMetaData($screen, $request, $object)
    {
        if (in_array(get_class($object), config('seohelper.supported', [])) && $request->has('seo_meta')) {
            try {
    ...
    {
        try {
            if (in_array(get_class($object), config('seohelper.supported', []))) {
                MetaBox::deleteMetaData($object, 'seo_meta');
            }
    ...
    public function registerModule($model)
    {
        ...
        config([
            'seohelper.supported' => array_merge(config('seohelper.supported', []),
                $model),
        ]);
