...
class RoleTable extends TableAbstract
{
     ...
     */
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, RoleInterface $roleRepository)
    {
        parent::__construct($table, $urlGenerator);

        $this->repository = $roleRepository;

        if (!Auth::user()->can(['role_edit', 'role_destroy'])) {
        ...
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (! Auth::user()->can('role_edit')) {
                    ...
                    $name = Html::link(route('role.edit', $item->id), DboardHelper::clean($item->name));
                }
            ...
            ->addColumn('operations', function ($item) {
                return $this->getOperations('role.edit', 'role.destroy', $item);
            });
    ...
    {
        return $this->addCreateButton(route('role.create'), 'role_create');
    }

    public function bulkActions(): array
    {
        return $this->addDeleteAction(route('role.deletes'), 'role_destroy', parent::bulkActions());
    }
