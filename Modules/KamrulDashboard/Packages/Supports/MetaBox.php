...
class MetaBox
{
    ...
    public function doMetaBoxes($context, $object = null)
    {
        ...
        if (isset($this->metaBoxes[$reference][$context])) {
            foreach (['high', 'sorted', 'core', 'default', 'low'] as $priority) {
                ...
                foreach ((array)$this->metaBoxes[$reference][$context][$priority] as $box) {
                    ...
                    $index++;
                    $data .= view('kamruldashboard::elements.meta-box-wrap', [
                        'box'      => $box,
        ...
        echo view('kamruldashboard::elements.meta-box', compact('data', 'context'))->render();
    ...
    public function getMetaPro($object, string $key, $select = ['meta_value'])
    {
        $key = apply_filters('stored_meta_box_key', $key, $object);

        $data = $this->metaBoxRepository->advancedGet([
            'condition' => [
                'meta_key'          => $key,
                'reference_type'    => $object,
                'meta_value'        => json_encode($select)
            ],
//            'take'      => $shortcode->number_of_mentor,
//            'meta_key'       => $key,
////            'reference_id'   => $object->id,
//            'reference_type' => $object,
        ]);
//        dd($data);
//        dd(json_encode($select));
        return $data;
    }
