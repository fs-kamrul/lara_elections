...
class DashboardWidgetInstance
{
    ...
    public function getPredefinedRangesDefault(): array
    {
        ...
        return [
            [
                'key'       => 'today',
                'label'     => trans('kamruldashboard::lang.predefined_ranges.today'),
                'startDate' => today()->startOfDay(),
                'endDate'   => $endDate,
            ],
            [
                'key'       => 'yesterday',
                'label'     => trans('kamruldashboard::lang.predefined_ranges.yesterday'),
                'startDate' => Carbon::yesterday()->startOfDay(),
                'endDate'   => Carbon::yesterday()->endOfDay(),
            ],
            [
                'key'       => 'this_week',
                'label'     => trans('kamruldashboard::lang.predefined_ranges.this_week'),
                'startDate' => now()->startOfWeek(),
                'endDate'   => now()->endOfWeek(),
            ],
            [
                'key'       => 'last_7_days',
                'label'     => trans('kamruldashboard::lang.predefined_ranges.last_7_days'),
                'startDate' => now()->subDays(7)->startOfDay(),
                'endDate'   => $endDate,
            ],
            [
                'key'       => 'this_month',
                'label'     => trans('kamruldashboard::lang.predefined_ranges.this_month'),
                'startDate' => now()->startOfMonth(),
                'endDate'   => $endDate,
            ],
            [
                'key'       => 'last_30_days',
                'label'     => trans('kamruldashboard::lang.predefined_ranges.last_30_days'),
                'startDate' => now()->subDays(29)->startOfDay(),
                'endDate'   => $endDate,
            ],
            [
                'key'       => 'this_year',
                'label'     => trans('kamruldashboard::lang.predefined_ranges.this_year'),
...
//        if (!Auth::user()->hasPermission($this->permission)) {
        ...
        if ($this->type === 'widget') {
            ...
            $data = [
                'id'   => $widget->id,
                'type' => $this->type,
                'view' => view('kamruldashboard::widgets.base', compact('widget', 'widgetSetting', 'settings', 'predefinedRanges'))->render(),
            ];
        ...
        $widgets[$this->key] = [
            'id'   => $widget->id,
            'type' => $this->type,
            'view' => view('kamruldashboard::widgets.stats', compact('widget', 'widgetSetting'))->render(),
        ];
