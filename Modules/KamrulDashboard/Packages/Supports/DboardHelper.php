...
use Modules\Icon\View\Components\Icon;
use Modules\KamrulDashboard\Packages\Components\BadgeComponent;
use Throwable;
...
class DboardHelper
{
    ...
    {
        if (empty($format)) {
            $format = config('kamruldashboard.date_format.date');
        }
    ...
    {
        if (empty($format)) {
            $format = config('kamruldashboard.date_format.date_time');
        }
    ...
    {
        return config('kamruldashboard.admin_dir');
    }

    public function getAdminMasterLayoutTemplate(): string
    {
        return apply_filters('base_filter_admin_master_layout_template', 'kamruldashboard::layouts.app_master');
    }
    ...
    {
        $richEditor = setting('rich_editor', config('kamruldashboard.editor.primary'));
    ...
    {
        return config('kamruldashboard.phone_validation_rule');
    }
    public function getHomepageUrl()
    {
        return apply_filters('cms_homepage_url', rescue(function () { return route('public.index'); }, false));
    }
    ...
    {
        if (config('kamruldashboard.enable_less_secure_web', false)) {
            return $dirty;
        }
    ...
    {
        return config('kamruldashboard.google_fonts_url', 'https://fonts.bunny.net');
    }

    public function googleFonts(string $font, bool $inline = true)
    {
        if (! config('kamruldashboard.google_fonts_enabled_cache')) {
        ...
        $font = app('kamruldashboard:google-fonts')->load($font);
