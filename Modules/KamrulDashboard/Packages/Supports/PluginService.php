...
class PluginService
{
    ...
    public function activate(string $plugin): array
    {
        ...
        if (empty($content)) {
            return [
                'error'   => true,
                'message' => trans('kamruldashboard::lang.invalid_json'),
            ];
        }
        ...
        if (!Arr::get($content, 'ready', 1)) {
            return [
                'error'   => true,
                'message' => trans('kamruldashboard::lang.plugin_is_not_ready',
        ...
        if (!in_array($plugin, $activatedPlugins)) {
            ...
            if (!empty(Arr::get($content, 'require'))) {
                ...
                if (!$valid) {
                    ...
                    return [
                        'error'   => true,
                        'message' => trans('kamruldashboard::lang.missing_required_plugins',
            ...
            return [
                'error'   => false,
                'message' => trans('kamruldashboard::lang.activate_success'),
            ];
        }

        return [
            'error'   => true,
            'message' => trans('kamruldashboard::lang.activated_already'),
        ];
    }
    ...
    protected function validate(string $plugin): array
    {
        ...
        if (!$this->files->isDirectory($location)) {
            return [
                'error'   => true,
                'message' => trans('kamruldashboard::lang.plugin_not_exist'),
            ];
        }
        ...
        if (!$this->files->exists($location . '/plugin.json')) {
            return [
                'error'   => true,
                'message' => trans('kamruldashboard::lang.missing_json_file'),
            ];
        }

        return [
            'error'   => false,
            'message' => trans('kamruldashboard::lang.plugin_invalid'),
        ];
    }
    ...
    public function publishAssets(string $plugin): array
    {
        ...
        $pluginPath = public_path('vendor/Modules');
        ...
        if (!$this->files->isWritable($pluginPath)) {
            return [
                'error'   => true,
                'message' => trans('kamruldashboard::lang.folder_is_not_writeable',
                    ['name' => $pluginPath]),
            ];
        }
        ...
        return [
            'error'   => false,
            'message' => trans('kamruldashboard::lang.published_assets_success', ['name' => $plugin]),
        ];
    }
    ...
    public function remove(string $plugin): array
    {
        ...
        return [
            'error'   => false,
            'message' => trans('kamruldashboard::lang.plugin_removed'),
        ];
    }
    ...
    public function deactivate(string $plugin): array
    {
        ...
        if (in_array($plugin, $activatedPlugins)) {
            ...
            return [
                'error'   => false,
                'message' => trans('kamruldashboard::lang.deactivated_success'),
            ];
        }

        return [
            'error'   => true,
            'message' => trans('kamruldashboard::lang.deactivated_already'),
        ];
    }
}
