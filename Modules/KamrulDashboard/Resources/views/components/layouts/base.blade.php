...
<head>
    ...
    @if (setting('admin_favicon') || config('kamruldashboard.base.general.favicon'))
        <link
            href="{{ $favicon = setting('admin_favicon') ? RvMedia::getImageUrl(setting('admin_favicon')) : url(config('kamruldashboard.base.general.favicon')) }}"
            rel="icon shortcut"
        >
        <meta
            property="og:image"
    ...
    <meta
        name="description"
        content="{{ $copyright = strip_tags(trans('kamruldashboard::layouts.copyright', ['year' => Carbon\Carbon::now()->year, 'company' => setting('admin_title', config('kamruldashboard.base.general.base_name')), 'version' => get_cms_version()])) }}"
    >
    <meta
    ...
    @include('kamruldashboard::components.layouts.header')
