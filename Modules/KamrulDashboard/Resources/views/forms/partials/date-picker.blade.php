@php
    ...
    $attributes['data-date-format'] = config('kamruldashboard.date_format.date');
    ...
    if ($value instanceof \Illuminate\Support\Carbon || $value instanceof \Carbon\Carbon) {
        $value = $value->format(config('kamruldashboard.date_format.date'));
    } elseif (is_string($value) && strtotime($value)) {
        $value = \Carbon\Carbon::parse($value)->format(config('kamruldashboard.date_format.date'));
    }
@endphp
...
<div class="input-group datepicker">
    ...
    <button class="btn btn-outline-secondary" type="button" title="Toggle calendar" data-toggle="datepicker">
    ...
    <button class="btn btn-outline-danger" type="button" title="Clear date" data-clear="datepicker">
        <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 17 17" width="16" height="16" fill="currentColor">
