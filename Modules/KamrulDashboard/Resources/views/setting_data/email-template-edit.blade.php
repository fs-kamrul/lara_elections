    ...
    <div class="max-width-1200">
        <div class="flexbox-annotated-section">
            <div class="flexbox-annotated-section-annotation">
                <div class="annotated-section-title pd-all-20">
                    <h2>{{ trans('kamruldashboard::setting.email.title') }}</h2>
                </div>
                <div class="annotated-section-description pd-all-20 p-none-t">
                    <p class="color-note">
                        {!! DboardHelper::clean(trans('kamruldashboard::setting.email.description')) !!}
                    </p>
                </div>
            </div>
            ...
            <div class="flexbox-annotated-section-content" v-pre>
                <div class="wrapper-content pd-all-20 email-template-edit-wrap">
                    @if ($emailSubject)
                        <div class="form-group mb-3">
                            <label class="text-title-field" for="email_subject">
                                {{ trans('kamruldashboard::setting.email.subject') }}
                            </label>
                    ...
                    @endif
                    <div class="form-group mb-3">
                        <label class="text-title-field" for="email_content">{{ trans('kamruldashboard::setting.email.content') }}</label>
        ...
        <div class="flexbox-annotated-section" style="border: none">
            ...
            <div class="flexbox-annotated-section-content">
                <a href="{{ route('settings.email') }}" class="btn btn-secondary">{{ trans('kamruldashboard::setting.email.back') }}</a>
                <a href="{{ route('setting.email.preview', ['type' => $pluginData['type'], 'module' => $pluginData['name'], 'template' => $pluginData['template_file']]) }}" target="_blank" class="btn btn-success">
                    {{ trans('kamruldashboard::setting.preview') }}
                    <i class="fa fa-external-link"></i>
                </a>
                <a class="btn btn-warning btn-trigger-reset-to-default text-white" data-target="{{ route('setting.email.template.reset-to-default') }}">{{ trans('kamruldashboard::setting.email.reset_to_default') }}</a>
                <button class="btn btn-info" type="submit" name="submit">{{ trans('kamruldashboard::setting.save_settings') }}</button>
            </div>
        </div>
    </div>
    {!! Form::close() !!}

    {!! Form::modalAction('reset-template-to-default-modal', trans('kamruldashboard::setting.email.confirm_reset'), 'info', trans('kamruldashboard::setting.email.confirm_message'), 'reset-template-to-default-button', trans('kamruldashboard::setting.email.continue')) !!}
@endsection
