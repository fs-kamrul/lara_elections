        ...
        <div class="wrapper-content pd-all-20">
            <div class="table-wrap">
                <table class="table product-list ws-nm">
                    <thead>
                    <tr>
                        <th class="border-none-b">{{ trans('kamruldashboard::setting.template') }}</th>
                        <th class="border-none-b"> {{ trans('kamruldashboard::setting.description') }} </th>
                        @if ($type !== 'core')
                            <th class="border-none-b text-center"> {{ trans('kamruldashboard::setting.enable') }}</th>
                        @else
                            <th>&nbsp;</th>
                        @endif
                    </tr>
                            ...
                            <td class="text-center template-setting-on-off">
                                @if ($type !== 'core' && Arr::get($template, 'can_off', false))
                                    <div class="form-group mb-3">
                                        {!! Form::onOffV2(get_setting_email_status_key($type, $module, $key),
                                ...
                                @elseif ($type !== 'core')
                                    <div class="form-group mb-3">
                                        {!! Form::onOffV2(get_setting_email_status_key($type, $module, $key), 1,
