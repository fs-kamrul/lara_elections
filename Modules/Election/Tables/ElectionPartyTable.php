...
class ElectionPartyTable extends TableAbstract
{
    /**
     * @var bool
     */
    ...
    /**
     * ElectionPartyTable constructor.
     * @param DataTables $table
     * @param UrlGenerator $urlGenerator
     * @param ElectionPartyInterface $electionpartyRepository
     */
    public function __construct(DataTables $table, UrlGenerator $urlGenerator, ElectionPartyInterface $electionpartyRepository)
    {
        parent::__construct($table, $urlGenerator);

        $this->repository = $electionpartyRepository;

        if (!Auth::user()->can(['electionparty_edit', 'electionparty_destroy'])) {
        ...
        $data = $this->table
            ...
            ->editColumn('name', function ($item) {
                if (! Auth::user()->can('electionparty_edit')) {
                    ...
                    $name = Html::link(route('electionparty.edit', $item->id), DboardHelper::clean($item->name));
                }

                return $name;
            })
            ->editColumn('photo', function ($item) {
                $photo_path = 'election/electionparty';
            ...
            ->addColumn('operations', function ($item) {
                return $this->getOperations('electionparty.edit', 'electionparty.destroy', $item);
            });
    ...
    {
        return $this->addCreateButton(route('electionparty.create'), 'electionparty_create');
    }

    public function bulkActions(): array
    {
        return $this->addDeleteAction(route('electionparty.deletes'), 'electionparty_destroy', parent::bulkActions());
    }
    ...
    public function getBulkChanges(): array
    {
        return [
            ...
            'status' => [
                'title' => trans('table::lang.status'),
                'type' => 'customSelect',
                'choices' => ElectionPartyStatusEnum::labels(),
                'validate' => 'required',
            ],
            'created_at' => [
                'title' => trans('table::lang.created_at'),
