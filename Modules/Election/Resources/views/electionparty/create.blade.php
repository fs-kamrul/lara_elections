...
@endsection
@section('title', __( 'election::lang.' . $title))
@section('content')

    @if(isset($record))
        @php
            $button = 'update';
            $language = getLanguageUrlPost($record->id , 'electionparty');
        @endphp
        <form name="formPage" method="POST" action="{{ $language['url'] }}" novalidate=""  enctype="multipart/form-data">
            <input type="hidden" name="model" value="{{ \Modules\Election\Http\Models\ElectionParty::class }}">
            <input type="hidden" name="language" value="{{ $language['code'] }}">
            @else
                @php
                    $button = 'save';
                @endphp
                <form method="POST" action="{{ route('electionparty.createelectionparty.store') }}"  enctype="multipart/form-data">
                    @endif
                    @csrf
                    @php do_action(BASE_ACTION_TOP_FORM_CONTENT_NOTIFICATION, request(), \Modules\Election\Http\Models\ElectionParty::class) @endphp

                    <div class="row">
                        <div class=" col-md-9">
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">@lang('election::lang.electionpartycreate')</h4>
                                </div>
                                <div class="card-body">
                                    <div class="basic-form">
                                        <div class="form-row">

                                            {!! Form::textField('name', isset($record) ? $record->name : null, 'election', '12', ['placeholder'=> __('election::lang.name')]) !!}

                                            @php
                                                if(isset($record)){
                                                    $slug = get_slug_table_data(null, \Modules\Election\Http\Models\ElectionParty::class,$record->id, \Modules\Election\Http\Models\ElectionParty::class);
                                                }
                                            @endphp
                                            {!! Form::permalink('slug', isset($slug) ? $slug->key : null, isset($slug) ? $slug->reference_id : null, isset($slug) ? $slug->prefix : null) !!}

                                            @isset($record) @if($language['code_edit']){!! input_design_html('photo',$record,12,'file', __('election::lang.photo')) !!} @endif @else{!! input_design_html('photo',null,6,'file', __('election::lang.photo')) !!} @endisset


                                            <div class="form-group col-md-12" id="short_description">
                                                <label>@lang('election::lang.short_description_or_seo_description')</label>
                                                {{--                                <input type="text" id="short_description" name="short_description" value="@isset($record){{$record->short_description}}@else{{ old('short_description') }}@endisset" class="form-control" placeholder="@lang('election::lang.short_description_or_seo_description')">--}}
                                                <textarea class="form-control" rows="4" placeholder="@lang('election::lang.short_description_or_seo_description')"
                                            ...
                                            <div class="col-xl-12 col-xxl-12 list-shortcode-items" id="content_set">
                                                <label><strong>@lang('election::lang.content')</strong></label>
                                                <div class="card">
                                                    <div class="card-body">
                                                        <div class="btn-group" role="group">
                                                            <button class="btn btn-primary show-hide-editor-btn" type="button" data-result="content">
                                                                {{ trans('election::lang.show_hide_editor') }}
                                                            </button>
                                                        </div>
                                                        <div class="btn-group" role="group">
                                                            <div class="basic-dropdown editor-action-item">
                                                                <div class="dropdown">
                                                                    <button type="button" class="btn btn-primary dropdown-toggle add_shortcode_btn_trigger" data-result="content" data-toggle="dropdown" data-bs-toggle="dropdown">
                                                                        <i class="fa fa-code"></i> {{ trans('election::lang.short_code') }}
                                                                    </button>
                                            ...
                                            <div class="col-xl-12 col-xxl-12" id="images_set" style="margin-top: 15px;">
                                                <label><strong>@lang('election::lang.images')</strong></label>
                            ...
                            @isset($record)
                                @php do_action(BASE_ACTION_META_BOXES, 'advanced', \Modules\Election\Http\Models\ElectionParty::where('id',$record->id)->first()) @endphp
                            @else
                                @php do_action(BASE_ACTION_META_BOXES, 'advanced', new \Modules\Election\Http\Models\ElectionParty ) @endphp
                            @endisset
                        </div>
                        <div class=" col-md-3">
                            @isset($record)
                            ...
                            {!! Form::formActions(__('election::lang.publish'), '') !!}

                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">@lang('election::lang.status')</h4>
                                </div>
                                <div class="card-body">
                                    <div class="basic-form">
                                        <div class="form-row">
                                            @isset($record)
                                                @if($language['code_edit'])
                                                    {!! status_admin_enum($record->status,12, '', \Modules\Election\Enums\ElectionPartyStatusEnum::class) !!}
                                                @endif @else {!! status_admin_enum(1,12, '' , \Modules\Election\Enums\ElectionPartyStatusEnum::class) !!}
                                            @endisset
                                        </div>
                                    </div>
                                </div>
                            ...
                            <div class="card">
                                <div class="card-header">
                                    <h4 class="card-title">@lang('election::lang.order')</h4>
                                </div>
                                <div class="card-body">
                                    <div class="basic-form">
                            ...
                            @isset($record)
                                @php do_action(BASE_ACTION_META_BOXES, 'top', \Modules\Election\Http\Models\ElectionParty::where('id',$record->id)->first()) @endphp
                            @else
                                @php do_action(BASE_ACTION_META_BOXES, 'top', new \Modules\Election\Http\Models\ElectionParty ) @endphp
                            @endisset
                        </div>
                    </div>
                </form>
