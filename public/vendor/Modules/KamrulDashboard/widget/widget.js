        ...
        let saveWidget = parentElement => {
            if (parentElement.length > 0) {
                ...
                $.ajax({
                    type: 'POST',
                    cache: false,
                    url: KDWidget.routes.save_widgets_sidebar,
                    data: {
                        items: items,
                        sidebar_id: parentElement.data('id')
                    },
                    beforeSend: () => {
                        kamruldashboard.showNotice('info', 'We are processing your request.');
                    },
                    success: data =>  {
                        if (data.error) {
                            ...
                            kamruldashboard.callScroll($('.list-page-select-widget'));
