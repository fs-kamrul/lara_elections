    ...
    if (type == "default") {
        ...
        for (let index = 0; index < checkbox.length; index++) {
            ...
            data.push({
                ...
                reference_id: element.attr('data-reference-id'),
                reference_type: element.attr('data-reference-type'),
                id: $('#idmenu').val()
            });
...
function actualizarMenu(serialize) {
    if ($('#menu-name').val()) {
        var menu_location_header_menu = new Array();
        $('input[name="locations"]:checked').each(function() {
            menu_location_header_menu.push($(this).val());
        });
        $.ajax({
            dataType: 'json',
            data: {
                ...
                idMenu: $('#idmenu').val(),
                location: menu_location_header_menu
            },
