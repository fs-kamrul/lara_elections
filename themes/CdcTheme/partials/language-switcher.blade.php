...
{{--                <i class="fa fa-chevron-down"></i>--}}
            ...
            <ul class="dropdown-menu language_dropdown {{ Arr::get($options, 'class') }}">
                @foreach ($supportedLocales as $localeCode => $properties)
                    @if ($localeCode != Language::getCurrentLocale())
                        <li>
                            <a class="dropdown-item lg-link" href="{{ Language::getSwitcherUrl($localeCode, $properties['lang_code']) }}">
    ...
    @else
        @foreach ($supportedLocales as $localeCode => $properties)
            @if ($localeCode != Language::getCurrentLocale())
                <div id="lang_form">
                    <a type="submit" class="language_style" href="{{ Language::getSwitcherUrl($localeCode, $properties['lang_code']) }}">
                        @if (Arr::get($options, 'lang_flag', true) && ($languageDisplay == 'all' || $languageDisplay == 'flag')){!! language_flag($properties['lang_flag'], $properties['lang_name']) !!}@endif
                        @if (Arr::get($options, 'lang_name', true) && ($languageDisplay == 'all' || $languageDisplay == 'name'))<span>{{ $properties['lang_name'] }}</span>@endif
                    </a>
                </div>
{{--                <li class="nav-item">--}}
{{--                    <a class="nav-link" href="{{ Language::getSwitcherUrl($localeCode, $properties['lang_code']) }}">--}}
{{--                        @if (Arr::get($options, 'lang_flag', true) && ($languageDisplay == 'all' || $languageDisplay == 'flag')){!! language_flag($properties['lang_flag'], $properties['lang_name']) !!}@endif--}}
{{--                        @if (Arr::get($options, 'lang_name', true) && ($languageDisplay == 'all' || $languageDisplay == 'name'))<span>{{ $properties['lang_name'] }}</span>@endif--}}
{{--                    </a>--}}
{{--                </li>--}}
